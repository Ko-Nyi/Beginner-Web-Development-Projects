...
function init(){
    ...
    childrenField = document.querySelector('#birthDate');
    birthDateField = document.querySelector('#children');
...
function checkName(){
  ...
  // in the past
birthDateField.addEventListener('input',checkDate);
...
//   // Listener for input events on the two text fields. Check for
//   // invalid charcters %, &, $, ! that are forbidden.
//   firstNameField.oninput = lastNameField.oninput = function() {
// 	if (this.value.match(/[%&$!]/)) {
// 		this.setCustomValidity(textMessageInvalid);
// 	} else {
// 		this.setCustomValidity("");
